...
export default class ERC721MintableRoyaltyExtend {
   ...
   */
  async deploy(params: DeployParams): Promise<ERC721MintableRoyaltyExtend> {
    if (this.contractAddress || this.contractDeployed) {
      log.throwArgumentError(
        ...
        {
          location: Logger.location.ERC721MINTABLEROYALTYEXTEND_DEPLOY,
        },
      );
    }
  ...
  loadContract(params: ContractAddressOptions): ERC721MintableRoyaltyExtend {
    if (this.contractAddress || this.contractDeployed) {
      log.throwArgumentError(
        ...
        this.contractAddress,
        {
          location: Logger.location.ERC721MINTABLEROYALTYEXTEND_LOADCONTRACT,
        },
      );
    }
  ...
  async setContractURI(
    ...
    if (!this.contractDeployed && !this.contractAddress) {
      log.throwArgumentError(
        ...
        this.contractAddress,
        {
          location: Logger.location.ERC721MINTABLEROYALTYEXTEND_SET_CONTRACT_URI,
        },
      );
    }
