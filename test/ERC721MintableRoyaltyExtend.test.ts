...
import ERC721MintableRoyaltyExtend from '../src/lib/ContractTemplates/ERC721MintableRoyaltyExtend';
import { ACCOUNT_ADDRESS, CONTRACT_ADDRESS, ACCOUNT_ADDRESS_2 } from './__mocks__/utils';
import version from '../src/_version';

let eRC721MintableRoyaltyExtend: ERC721MintableRoyaltyExtend;
let signer: string;
...
describe('SDK', () => {
    ...
    () =>
      ({
        deployed: () => ({
          ...
          approve: jest.fn(),
          setDefaultRoyalty: jest.fn(),
          deleteDefaultRoyalty: jest.fn(),
          setTokenRoyalty: jest.fn(),
  ...
  it('should create "ERC721MintableRoyaltyExtend" instance', () => {
    eRC721MintableRoyaltyExtend = new ERC721MintableRoyaltyExtend(
      signer as unknown as ethers.Wallet,
    );

    expect(eRC721MintableRoyaltyExtend).not.toBe(null);
  });

  it('[Deploy] - should return an Error if signer not defined ', () => {
    eRC721MintableRoyaltyExtend = new ERC721MintableRoyaltyExtend(null as unknown as ethers.Wallet);

    const contract = async () =>
      eRC721MintableRoyaltyExtend.deploy({ name: 'name', symbol: 'symbol', contractURI: 'URI' });

    expect(contract).rejects.toThrow(
      `No signer instance supplied. (location="[ERC721MintableRoyaltyExtend.deploy]", argument="signer", value=null, code=INVALID_ARGUMENT, version=${version})`,
    );
  });
  ...
  it('[Deploy] - should return an Error if Name is empty', () => {
    ...
    const contract = async () =>
      eRC721MintableRoyaltyExtend.deploy({ name: '', symbol: 'symbol', contractURI: 'URI' });

    expect(contract).rejects.toThrow(
      `No name supplied. (location="[ERC721MintableRoyaltyExtend.deploy]", argument="name", value="", code=INVALID_ARGUMENT, version=${version})`,
    );
  });
  ...
  it('[Mint] - should return an Error if contract is not deployed', () => {
    ...
    const myNFT = async () =>
      eRC721MintableRoyaltyExtend.mint({
        publicAddress: ACCOUNT_ADDRESS,
        tokenURI: 'https://infura.io/images/404.png',
      });
    expect(myNFT).rejects.toThrow(
      `Contract not deployed. (location="[ERC721MintableRoyaltyExtend.mint]", argument="contractAddress", value=undefined, code=INVALID_ARGUMENT, version=${version})`,
    );
  });
  ...
  it('[Mint] - should console.warn if tokenURI is not a link ', async () => {
    ...
    await eRC721MintableRoyaltyExtend.mint({
  ...
  it('[Mint] - should return an Error if the address is empty', () => {
    ...
    expect(myNFT).rejects.toThrow(
      `missing argument: Invalid public address. (location="[ERC721MintableRoyaltyExtend.mint]", code=MISSING_ARGUMENT, version=${version})`,
    );
  });
  ...
  it('[Mint] - should return an Error if the tokenURI is empty', () => {
    ...
    expect(myNFT).rejects.toThrow(
      `missing argument: No tokenURI supplied. (location="[ERC721MintableRoyaltyExtend.mint]", code=MISSING_ARGUMENT, version=${version})`,
    );
  });
  ...
  it('[LoadContract] - should return an Error if contract is already deployed', () => {
    ...
    const contract = async () => {
      ...
      await eRC721MintableRoyaltyExtend.loadContract({ contractAddress: CONTRACT_ADDRESS });
    };
    expect(contract).rejects.toThrow(
      `Contract already deployed. (location="[ERC721MintableRoyaltyExtend.loadContract]", argument="contractAddress", value=undefined, code=INVALID_ARGUMENT, version=${version})`,
    );
  });
  ...
  it('[LoadContract] - should return an Error if the address is empty', () => {
    ...
    const contract = async () => {
      await eRC721MintableRoyaltyExtend.loadContract({ contractAddress: '' });
    };
    expect(contract).rejects.toThrow(
      `missing argument: Invalid contract address. (location="[ERC721MintableRoyaltyExtend.loadContract]", code=MISSING_ARGUMENT, version=${version})`,
    );
  });
  ...
  it('[Transfer] - should return an Error if contract is not deployed', () => {
    ...
    const transferNft = async () =>
      eRC721MintableRoyaltyExtend.baseERC721.transfer({
  ...
  it('[Transfer] - should return an Error if from address is not valid', () => {
    ...
    const transferNft = async () => {
      ...
      await eRC721MintableRoyaltyExtend.baseERC721.transfer({
        from: '',
        to: ACCOUNT_ADDRESS_2,
        tokenId: 1,
      });
  ...
  it('[SetContractURI] - should return an Error if contract is not deployed', () => {
    ...
    expect(() =>
      eRC721MintableRoyaltyExtend.setContractURI({
        contractURI:
          'https://www.cryptotimes.io/wp-content/uploads/2022/03/BAYC-835-Website-800x500.jpg',
      }),
    ).rejects.toThrow(
      `Contract not deployed. (location="[ERC721MintableRoyaltyExtend.setContractURI]", argument="contractAddress", value=undefined, code=INVALID_ARGUMENT, version=${version})`,
    );
  });
  ...
  it('[SetContractURI] - should console.warn if contractURI is not a link ', async () => {
    ...
    await eRC721MintableRoyaltyExtend.setContractURI({ contractURI: 'URI' });
  ...
  it('[SetContractURI] - should return an Error if the contractURI is empty', () => {
    ...
    const uri = async () => {
      ...
      await eRC721MintableRoyaltyExtend.setContractURI({ contractURI: '' });
    };
    expect(uri).rejects.toThrow(
      `missing argument: No contractURI supplied. (location="[ERC721MintableRoyaltyExtend.setContractURI]", code=MISSING_ARGUMENT, version=${version})`,
    );
  });
  ...
  it('[SetContractURI] - should set the contractURI', async () => {
    ...
    await eRC721MintableRoyaltyExtend.setContractURI({
      contractURI:
        'https://www.cryptotimes.io/wp-content/uploads/2022/03/BAYC-835-Website-800x500.jpg',
    });
  ...
  it('[addMinter] - should return an Error if contract is not deployed', () => {
    ...
    const minter = async () =>
      eRC721MintableRoyaltyExtend.accessControl.addMinter({ publicAddress: ACCOUNT_ADDRESS });
  ...
  it('[addMinter] - should return an Error if the address is empty', () => {
    ...
    const minter = async () => {
      ...
      await eRC721MintableRoyaltyExtend.accessControl.addMinter({ publicAddress: '' });
    };
    expect(minter).rejects.toThrow(
  ...
  it('[addMinter] - should add minter role to an address', async () => {
    ...
    await eRC721MintableRoyaltyExtend.accessControl.addMinter({ publicAddress: ACCOUNT_ADDRESS });
  ...
  it('[removeMinter] - should return an Error if contract is not deployed', () => {
    ...
    const minter = async () =>
      eRC721MintableRoyaltyExtend.accessControl.removeMinter({ publicAddress: ACCOUNT_ADDRESS });
    expect(minter).rejects.toThrow(
  ...
  it('[removeMinter] - should return an Error if the address is empty', () => {
    ...
    const minter = async () => {
      ...
      await eRC721MintableRoyaltyExtend.accessControl.removeMinter({ publicAddress: '' });
    };
    expect(minter).rejects.toThrow(
  ...
  it('[removeMinter] - should remove minter role to an address', async () => {
    ...
    await eRC721MintableRoyaltyExtend.accessControl.removeMinter({
      publicAddress: ACCOUNT_ADDRESS,
    });
  ...
  it('[renounceMinter] - should return an Error if contract is not deployed', () => {
    ...
    const minter = async () =>
      eRC721MintableRoyaltyExtend.accessControl.renounceMinter({ publicAddress: ACCOUNT_ADDRESS });
    expect(minter).rejects.toThrow(
  ...
  it('[renounceMinter] - should return an Error if the address is empty', () => {
    ...
    const minter = async () => {
      ...
      await eRC721MintableRoyaltyExtend.accessControl.renounceMinter({ publicAddress: '' });
    };
    expect(minter).rejects.toThrow(
  ...
  it('[renounceMinter] - should renounce minter role for an address', async () => {
    ...
    await eRC721MintableRoyaltyExtend.accessControl.renounceMinter({
      publicAddress: ACCOUNT_ADDRESS,
    });
  ...
  it('[isMinter] - should return an Error if contract is not deployed', () => {
    ...
    const minter = async () =>
      eRC721MintableRoyaltyExtend.accessControl.isMinter({ publicAddress: ACCOUNT_ADDRESS });
    expect(minter).rejects.toThrow(
  ...
  it('[isMinter] - should return an Error if the address is empty', () => {
    ...
    const minter = async () => {
      ...
      await eRC721MintableRoyaltyExtend.accessControl.isMinter({ publicAddress: '' });
    };
    expect(minter).rejects.toThrow(
  ...
  it('[isMinter] - should check if an address has minter role', async () => {
    ...
    await eRC721MintableRoyaltyExtend.accessControl.isMinter({ publicAddress: ACCOUNT_ADDRESS });
  ...
  it('[SetApprovalForAll] - should return an Error if contract is not deployed', () => {
    ...
    expect(() =>
      eRC721MintableRoyaltyExtend.baseERC721.setApprovalForAll({
        to: ACCOUNT_ADDRESS,
        approvalStatus: true,
      }),
    ).rejects.toThrow(
  ...
  it('[SetApprovalForAll] - should return an Error if the address is empty', () => {
    ...
    const approval = async () => {
      ...
      await eRC721MintableRoyaltyExtend.baseERC721.setApprovalForAll({
        to: '',
        approvalStatus: true,
      });
    };
  ...
  it('[addAdmin] - should return an Error if contract is not deployed', () => {
    ...
    expect(() =>
      eRC721MintableRoyaltyExtend.accessControl.addAdmin({
  ...
  it('[addAdmin] - should return an Error because of bad address', () => {
    ...
    const admin = async () => {
      ...
      await eRC721MintableRoyaltyExtend.accessControl.addAdmin({ publicAddress: '' });
    };
  ...
  it('[addAdmin] - should add admin', async () => {
    ...
    await eRC721MintableRoyaltyExtend.accessControl.addAdmin({
  ...
  it('[removeAdmin] - should return an Error if contract is not deployed', () => {
    ...
    expect(() =>
      eRC721MintableRoyaltyExtend.accessControl.removeAdmin({
        publicAddress: '0xB3C24BB465b682225F8C87b29a031921B764Ed94',
      }),
    ).rejects.toThrow(
  ...
  it('[removeAdmin] - should return an Error because of bad address', () => {
    ...
    const admin = async () => {
      ...
      await eRC721MintableRoyaltyExtend.accessControl.removeAdmin({ publicAddress: '' });
    };
    expect(admin).rejects.toThrow(
  ...
  it('[removeAdmin] - should remove admin', async () => {
    ...
    await eRC721MintableRoyaltyExtend.accessControl.removeAdmin({
      publicAddress: '0x417C0309d43C27593F8a4DFEC427894306f6CE67',
    });
  ...
  it('[renounceAdmin] - should return an Error if contract is not deployed', () => {
    ...
    expect(() =>
      eRC721MintableRoyaltyExtend.accessControl.renounceAdmin({
        publicAddress: '0xB3C24BB465b682225F8C87b29a031921B764Ed94',
      }),
    ).rejects.toThrow(
  ...
  it('[renounceAdmin] - should return an Error because of bad address', () => {
    ...
    const admin = async () => {
      ...
      await eRC721MintableRoyaltyExtend.accessControl.renounceAdmin({ publicAddress: '' });
    };
    expect(admin).rejects.toThrow(
  ...
  it('[renounceAdmin] - should renounce admin', async () => {
    ...
    await eRC721MintableRoyaltyExtend.accessControl.renounceAdmin({
      publicAddress: '0x417C0309d43C27593F8a4DFEC427894306f6CE67',
    });
  ...
  it('[isAdmin] - should return an Error if contract is not deployed', () => {
    ...
    expect(() =>
      eRC721MintableRoyaltyExtend.accessControl.isAdmin({
        publicAddress: '0xB3C24BB465b682225F8C87b29a031921B764Ed94',
      }),
    ).rejects.toThrow(
  ...
  it('[isAdmin] - should return an Error because of bad address', () => {
    ...
    const admin = async () => {
      ...
      await eRC721MintableRoyaltyExtend.accessControl.isAdmin({ publicAddress: '' });
    };
    expect(admin).rejects.toThrow(
  ...
  it('[isAdmin] - should renounce admin', async () => {
    ...
    await eRC721MintableRoyaltyExtend.accessControl.isAdmin({
      publicAddress: '0x417C0309d43C27593F8a4DFEC427894306f6CE67',
    });
    expect(contractFactoryMock).toHaveBeenCalledTimes(1);
  });
  ...
  it('[ApproveTransfer] - should return an Error if contract is not deployed', () => {
    ...
    const approveTransfer = async () =>
      eRC721MintableRoyaltyExtend.baseERC721.approveTransfer({ to: ACCOUNT_ADDRESS_2, tokenId: 1 });
  ...
  it('[ApproveTransfer] - should return an Error if to address is not valid', async () => {
    ...
    const approveTransfer = async () => {
      ...
      await eRC721MintableRoyaltyExtend.baseERC721.approveTransfer({ to: '', tokenId: 1 });
    };
  ...
  it('[ApproveTransfer] - should approve transfer nft', async () => {
    ...
    await eRC721MintableRoyaltyExtend.baseERC721.approveTransfer({
      to: ACCOUNT_ADDRESS,
      tokenId: 1,
    });
  ...
  describe('setDefaultRoyalty', () => {
    it('[setDefaultRoyalty] - should throw if contract not deployed', async () => {
      const contract = new ERC721MintableRoyaltyExtend(signer as unknown as ethers.Wallet);

      await expect(() =>
        contract.royalty.setDefaultRoyalty({ publicAddress: ACCOUNT_ADDRESS, fee: 1 }),
      ).rejects.toThrow(
        `Contract not deployed. (location="[HasRoyaltyExtend.setDefaultRoyalty]", argument="contractAddress", value=undefined, code=INVALID_ARGUMENT, version=${version})`,
      );
    });

    it('[setDefaultRoyalty] - should throw when "fee" is not a number larger than or equal 0 and less than or equal 10000', async () => {
      const contract = new ERC721MintableRoyaltyExtend(signer as unknown as ethers.Wallet);
      ...
      await expect(() =>
        ...
        `Fee must be between 0 and 10000. (location="[HasRoyaltyExtend.setDefaultRoyalty]", argument="fee", value=-1, code=INVALID_ARGUMENT, version=${version})`,
      );

      await expect(() =>
        contract.royalty.setDefaultRoyalty({ publicAddress: ACCOUNT_ADDRESS, fee: 10001 }),
      ).rejects.toThrow(
        `Fee must be between 0 and 10000. (location="[HasRoyaltyExtend.setDefaultRoyalty]", argument="fee", value=10001, code=INVALID_ARGUMENT, version=${version})`,
      );
    });

    it('[setDefaultRoyalty] - should set royalties', async () => {
      const contract = new ERC721MintableRoyaltyExtend(signer as unknown as ethers.Wallet);
      await contract.deploy({ name: 'name', symbol: 'symbol', contractURI: 'URI' });
      contract.royalty.setDefaultRoyalty({ publicAddress: ACCOUNT_ADDRESS, fee: 0 });
      await expect(contractFactoryMock).toHaveBeenCalledTimes(1);
      contract.royalty.setDefaultRoyalty({ publicAddress: ACCOUNT_ADDRESS, fee: 10000 });
      await expect(contractFactoryMock).toHaveBeenCalledTimes(1);
    });
  });

  describe('deleteDefaultRoyalty', () => {
    it('[deleteDefaultRoyalty] - should throw if contract not deployed', async () => {
      const contract = new ERC721MintableRoyaltyExtend(signer as unknown as ethers.Wallet);

      await expect(() => contract.royalty.deleteDefaultRoyalty({})).rejects.toThrow(
        `Contract not deployed. (location="[HasRoyaltyExtend.deleteDefaultRoyalty]", argument="contractAddress", value=undefined, code=INVALID_ARGUMENT, version=${version})`,
      );
    });

    it('[deleteDefaultRoyalty] - should set royalties', async () => {
      const contract = new ERC721MintableRoyaltyExtend(signer as unknown as ethers.Wallet);
      await contract.deploy({ name: 'name', symbol: 'symbol', contractURI: 'URI' });
      contract.royalty.deleteDefaultRoyalty({});
      await expect(contractFactoryMock).toHaveBeenCalledTimes(1);
    });
  });

  describe('setTokenRoyalty', () => {
    it('[setTokenRoyalty] - should throw if contract not deployed', async () => {
      const contract = new ERC721MintableRoyaltyExtend(signer as unknown as ethers.Wallet);

      await expect(() =>
        contract.royalty.setTokenRoyalty({ tokenId: 0, publicAddress: ACCOUNT_ADDRESS, fee: 1 }),
      ).rejects.toThrow(
        `Contract not deployed. (location="[HasRoyaltyExtend.setTokenRoyalty]", argument="contractAddress", value=undefined, code=INVALID_ARGUMENT, version=${version})`,
      );
    });

    it('[setTokenRoyalty] - should throw when "fee" is not a number larger than or equal 0 and less than or equal 10000', async () => {
      const contract = new ERC721MintableRoyaltyExtend(signer as unknown as ethers.Wallet);
      await contract.deploy({ name: 'name', symbol: 'sumbol', contractURI: 'URI' });
      ...
      await expect(() =>
        ...
        `Fee must be between 0 and 10000. (location="[HasRoyaltyExtend.setTokenRoyalty]", argument="fee", value=-1, code=INVALID_ARGUMENT, version=${version})`,
      );

      await expect(() =>
        contract.royalty.setTokenRoyalty({
          tokenId: 0,
          publicAddress: ACCOUNT_ADDRESS,
          fee: 10001,
        }),
      ).rejects.toThrow(
        `Fee must be between 0 and 10000. (location="[HasRoyaltyExtend.setTokenRoyalty]", argument="fee", value=10001, code=INVALID_ARGUMENT, version=${version})`,
      );
    });

    it('[setTokenRoyalty] - should set royalties', async () => {
      const contract = new ERC721MintableRoyaltyExtend(signer as unknown as ethers.Wallet);
      await contract.deploy({ name: 'name', symbol: 'symbol', contractURI: 'URI' });
      contract.royalty.setTokenRoyalty({ tokenId: 0, publicAddress: ACCOUNT_ADDRESS, fee: 0 });
      await expect(contractFactoryMock).toHaveBeenCalledTimes(1);
      contract.royalty.setTokenRoyalty({ tokenId: 0, publicAddress: ACCOUNT_ADDRESS, fee: 10000 });
      await expect(contractFactoryMock).toHaveBeenCalledTimes(1);
    });
  });

  describe('resetTokenRoyalty', () => {
    it('[resetTokenRoyalty] - should throw if contract not deployed', async () => {
      const contract = new ERC721MintableRoyaltyExtend(signer as unknown as ethers.Wallet);

      await expect(() => contract.royalty.resetTokenRoyalty({ tokenId: 0 })).rejects.toThrow(
        `Contract not deployed. (location="[HasRoyaltyExtend.resetTokenRoyalty]", argument="contractAddress", value=undefined, code=INVALID_ARGUMENT, version=${version})`,
      );
    });

    it('[resetTokenRoyalty] - should set royalties', async () => {
      const contract = new ERC721MintableRoyaltyExtend(signer as unknown as ethers.Wallet);
      await contract.deploy({ name: 'name', symbol: 'symbol', contractURI: 'URI' });
      contract.royalty.resetTokenRoyalty({ tokenId: 0 });
      await expect(contractFactoryMock).toHaveBeenCalledTimes(1);
    });
  });
  ...
  describe('renounceOwnership', () => {
    it('[renounceOwnership] - should throw if contract not deployed', async () => {
      ...
      const renounceOwnership = async () =>
        eRC721MintableRoyaltyExtend.baseERC721.renounceOwnership({});
    ...
    it('[renounceOwnership] - should call renounce ownership', async () => {
      ...
      await eRC721MintableRoyaltyExtend.baseERC721.renounceOwnership({});
